...
if (isset($_GET['restore'])) {
    ...
    if ($row) {
        $stmt = $conn->prepare("INSERT INTO problems (title, path, category, type, question, answer, solution, difficulty, video, hint) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssssssss", $row['title'], $row['path'], $row['category'], $row['type'], $row['question'], $row['answer'], $row['solution'], $row['difficulty'], $row['video'], $row['hint']);
        $stmt->execute();
        $stmt->close();
...
<body>
    ...
    <table>
        <tr>
            <th>ID</th><th>제목</th><th>삭제일시</th><th>복구</th><th>영구삭제</th>
        </tr>
