<?php
require_once("/var/www/config/db_connect.php"); // PDO 방식 DB 연결 포함
...
try {
    ...
    // 1. 하위 경로가 있는지 확인
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM source_path WHERE parent_id = ?");
    $stmt->execute([$id]);
    ...
    // 2. 실제 삭제
    $stmt = $pdo->prepare("DELETE FROM source_path WHERE id = ?");
