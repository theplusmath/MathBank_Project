<?php
file_put_contents(__DIR__.'/php-error.log', date('c')." [reorder_path] 진입\n", FILE_APPEND);

ini_set('display_errors', 1);
ini_set('display_startup_errors', 1);
error_reporting(E_ALL);

require_once("/var/www/config/db_connect.php"); // PDO 객체 $pdo 제공
header('Content-Type: application/json');

try {
    $input = json_decode(file_get_contents("php://input"), true);
    $dragged_id = intval($input['dragged_id'] ?? 0);
    $target_id = intval($input['target_id'] ?? 0); // 사실상 안 씀
    ...
    $stmt = $pdo->prepare("SELECT parent_id FROM source_paths WHERE id = ?");
    $stmt->execute([$dragged_id]);
    $old_parent_id = $stmt->fetchColumn();

    // parent_id 변경 (드래그된 항목을 새 부모로 이동)
    $stmt = $pdo->prepare("UPDATE source_paths SET parent_id = ? WHERE id = ?");
    $stmt->execute([$new_parent_id, $dragged_id]);

    // 새 부모 하위 항목들 정렬 (자기 자신 제외)
    $stmt = $pdo->prepare("SELECT id FROM source_paths WHERE parent_id = ? AND id != ? ORDER BY sort_order ASC, id ASC");
    $stmt->execute([$new_parent_id, $dragged_id]);
    ...
    foreach ($new_order as $i => $id) {
        $stmt = $pdo->prepare("UPDATE source_paths SET sort_order = ? WHERE id = ?");
        $stmt->execute([$i + 1, $id]);
    }
    $pdo->commit();
