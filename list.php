...
if (isset($_GET['delete'])) {
    ...
    if ($row) {
        ...
        $stmt = $conn->prepare("INSERT INTO deleted_problems (original_id, title, path, category, type, question, answer, solution, difficulty, video, hint, source, deleted_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
        $stmt->bind_param("isssssssssss", $row['id'], $row['title'], $row['path'], $row['category'], $row['type'], $row['question'], $row['answer'], $row['solution'], $row['difficulty'], $row['video'], $row['hint'], $row['source']);
        $stmt->execute();
        $stmt->close();
